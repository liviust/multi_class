...
def _convert_to_example(filename, image_buffer, label, text, height, width):
  ...
  example = tf.train.Example(features=tf.train.Features(feature={
      ...
      'image/label': _int64_feature_list(label),
      'image/text': _bytes_feature_list(text),
      'image/filename': _bytes_feature(os.path.basename(filename)),
      'image/encoded': _bytes_feature(image_buffer)}))
  return example


def my_convert_to_example(filename, label, text, coder):
  ...
  context = tf.train.Features(feature={
      'image/filename': _bytes_feature(os.path.basename(filename)),
      'image/data': _bytes_feature(image_buffer),
      'image/label_fix': _int64_feature(label)
  })
...
def _process_image_files_batch(coder, thread_index, ranges, name, filenames,
  ...
  for s in xrange(num_shards_per_batch):
    ...
    for i in files_in_shard:
      ...
      example = my_convert_to_example(filename, label, text, coder)
...
def _process_image_files(name, filenames, texts, labels, num_shards):
  ...
  Args:
    ...
    filenames: list of strings; each string is a path to an image file
    texts: list of strings; each string is human readable, e.g. 'dog'
    labels: list of integer belong to {0, 1}; each integer identifies if this class is in the image
    num_shards: integer number of shards for this data set.
  """
...
def _prase_ground_true(data_root, label_list, class_list):
  """Build a list of all images files and labels in the data set.

  Args:
      data_root: string, path to the root directory of the images
      label_list: string, path to the multi-label file
      class_list: string, path to the class names file

      Assumes that the label list resides with
      the following directory structure.

        data_root/xxx.jpg dog bike
        data_root/xxx.jpg person bottle bottle

        where 'dog, bottle, person' are the labels associated with this image

  Returns:
      filenames: list of strings; each string is a path to an image file.
      texts: list of strings; each string is the class, e.g. 'dog'
      labels: list of numpy array; each array identifies the ground truth;
              each image may have more than one label
  """
  ...
  for i, sample in enumerate(samples):
    ...
    texts.append(_label)
    labels.append(gt_classes)

    if not i % 1000:
        print('Finished prasing %d samples.' % i)
  ...
  print('Found %d JPEG files across %d labels inside %s.' %
        (len(filenames), len(samples), data_root))
  return filenames, texts, labels


def my_process_dataset(name, directory, num_shards, label_list, class_list):
    filenames, texts, labels = _prase_ground_true(directory, label_list, class_list)
